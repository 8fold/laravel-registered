...
use Mail;
use Eightfold\Registered\Registration\UserRegisteredMailable;
...
class UserRegistration extends Model
{
    ...
    protected $dates = ['created_at', 'updated_at', 'confirmed_on'];
    ...
    static public function registerUser(string $username, string $email, UserType $type = null, UserInvitation $invitation = null): UserRegistration
    {
        ...
        if (!\App::runningUnitTests()) {
            Mail::to($registration->defaultEmailString)
                ->send(new UserRegisteredMailable($registration));
        }
     ...
     */
    static public function hasSiteOwner(): bool
    {
    ...
    /**
     * @todo Put somewhere else. Should not be in data object model.
     *
     * @return [type] [description]
     */
    public function getConfirmUrlAttribute(): string
    {
        return $this->profilePath .'/confirm-registration?token='. $this->token;
    }
