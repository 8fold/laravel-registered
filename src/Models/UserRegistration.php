...
class UserRegistration extends Model
{
    ...
    static public function registrationForm(Request $request): array
    {
        $inviteToken = (isset($request->token));
        $inviteRequired = config('registered.invitations.required');
        $inviteRequestable = config('registered.invitations.requestable');
        $hasOwner = UserRegistration::hasOwner();
        $useInviteRequestForm = ($inviteRequired && $hasOwner && $inviteRequestable && !$inviteToken);
        ...
        if ($useInviteRequestForm) {
            // TODO: Should probably put this on the invitation model.
            return [
                'attributes' => [
                    'action' => url('/register/request-invite')
                ],
                [
        ...
        if ($inviteRequired && $hasOwner) {
            $invitationTokenInput = [
                'element' => 'Eightfold\UIKit\UIKit::textinput',
                'label' => trans('registered::registration.invitation_code'),
                'name' => 'invite_code',
                'error' => ($errors->has('invite_code'))
        ...
        $tosLink = '';
        if (strlen(config('registered.tos_url')) > 0) {
            $tosLink = [
                'element' => 'a',
                'attributes' => [
                    'href' => url(config('registered.tos_url'))
                ],
        ...
        return [
            ...
            'content' => [
                ...
                $invitationTokenInput,
                [
                    'element' => 'Eightfold\UIKit\UIKit::formselect',
                    ...
                    'extra' => $tosLink,
                    'error' => ($errors->has('tos_acceptance'))
                        ? $errors->first('tos_acceptance')
                        : ''
                ],
                [
                    'element' => 'Eightfold\UIKit\UIKit::formbutton',
